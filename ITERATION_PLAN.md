...
| [ ] | **P1-2 与官方多用户账号联动** | WS 握手时校验 ST 会话 Cookie（多用户模式下），昵称默认取账号 Persona 名，未登录拒绝升级 |
| [ ] | **P1-3 注入后自动回应（可选开关）** | P1-1 遗留：注入真人消息后可自动触发一次 `Generate`（需防多成员同时触发的请求风暴：仅房主/轮值响应，或带冷却的全局节流），实现"AI 真正实时回应" |

## P2 体验优化

| 状态 | 任务 | 说明 |
|------|------|------|
| [ ] | **P2-1 房间历史持久化** | 房间消息落盘到 `data/default-user/sillyroom/`，服务器重启后可回溯，加入时回放最近 N 条 |
| [ ] | **P2-2 离线补发与未读** | 断线期间消息缓存补发；窗口最小化时未读角标 |
